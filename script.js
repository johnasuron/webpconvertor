document.addEventListener("DOMContentLoaded", () => {
    ...
    // Function to convert JPG to WEBP with custom dimensions and size percentage
    ...
    imageInput.addEventListener("change", (event) => {
        ...
        if (files.length > 0 && files.every((file) => file.type === "image/jpeg")) {
            convertButton.disabled = false;
            statusMessage.textContent = "";
        } else {
            convertButton.disabled = true;
            statusMessage.textContent = "Please select up to 5 valid JPG files.";
        }
    });
    ...
    convertButton.addEventListener("click", async () => {
        ...
        if (files.length === 0) {
            statusMessage.textContent = "Please select at least one JPG file.";
            return;
        }
